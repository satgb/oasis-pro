...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(inactiveTimer, &QTimer::timeout, this, &MainWindow::powerChange);
...
void MainWindow::initSession(Session* s)    //NOTE: needs testing
{
    if(profile->batteryLvl < 12.5){
        ui->console->append("Battery too low to run a session");
        powerChange();
        return;
    }
...
{
    //battery lasts for 5-15 minutes - longest session is 4.5 minutes
